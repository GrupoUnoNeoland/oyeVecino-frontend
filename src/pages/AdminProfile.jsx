...
export const AdminProfile = () => {
  ...
  const handleClickRequest = async (state, requestId, userId) => {
    ...
    const formDataNeighborhoodUser = JSON.stringify({
      neighborhoods: neighborhoodSelected,
    });
    const formDataCityUser = JSON.stringify({ city: citySelected });
    const formDataNeighborhoodRequest = JSON.stringify({
      neighborhoodId: neighborhoodSelected,
    });
    const formDataCityRequest = JSON.stringify({ cityId: citySelected });
    const formDataUserUpdate = JSON.stringify({
      ...
      {requests.length > 0 &&
        requests.map((request) => (
          <div key={request._id} className="request_container">
            <img
              className="request__user-photo"
              src={request.user[0].image}
              alt={request.user[0].name}
            />
            <div className="request-info">
              <p>usuario: {request.user[0].name}</p>
              <p>state: {request.state}</p>
              <a href={request.document} target="_blank">
                Documento
              </a>
            </div>
            <div className="request-btns">
              <button
                onClick={() =>
                  handleClickRequest('accepted', request._id, request.user[0]._id)
                }
              >
                Aprobar
              </button>
              <button
                onClick={() =>
                  handleClickRequest('rejected', request._id, request.user[0]._id)
                }
              >
                Rechazar
              </button>
            ...
            <div className="request__city-neighborhood-buttons">
              <div className="city_select">
                <label htmlFor="city" className="city_select__label  select-label">
                  Ciudade
                </label>
                <select
                  id="city_select__input"
                  name="city"
              ...
              <div className="neighborhood_select">
                <label
                  ...
                  className="neighborhood_select__label select-label"
                >
                  Barrio
                </label>
                <select
                  id="neighborhood_select__input"
                  onClick={(e) => setNeighborhoodSelected(e.target.value)}
                  name="neighborhood"
